/*
 * csmsemoa.c:
 *  This file implements the main procedures of constraint SMS-EMOA. It is based on the following reference:
 *
...
void CSMSEMOA (population_real *parent_pop, population_real *offspring_pop, population_real *mixed_pop)
{
    ...
    while (evaluation_count < max_evaluation)
    {
        ...
        for (i = 0; i < popsize; i++)
        {
            ...
            fill_constraint_hv_sort (f, parent_pop, mixed_pop, popsize + 1);
        }
        // track the current evolutionary progress, including population and metrics
