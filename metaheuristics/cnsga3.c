...
void association_constraint (population_real *pop, int size)
{
    int i, j, k, min_idx;
...
void assign_rank_constraint (population_real *pop, int size)
{
    int i, j;
    int flag, end;
    int front_size = 0;
...
void fill_nd_pop_constraint (population_real *mixed_pop, population_real *new_pop)
{
    int rank;
    list *temp;
...
void getExtremePoints_constraint (population_real *pop, int size)
{
...
double* gaussianElimination_constraint (double **A, double *b, double *x)
{
    int i, j, p;
    int N, max;
    double alpha, sum, t;
...
void niching_constraint (population_real *mix_pop, population_real *new_pop)
{
    int i, j, rand;
...
void getIntercepts_constraint (population_real *pop, int size)
{
...
void CNSGA3 (population_real *parent_pop, population_real *offspring_pop, population_real *mixed_pop)
{
    ...
    assign_rank_constraint(parent_pop, popsize);

    getExtremePoints_constraint (parent_pop, popsize);
    getIntercepts_constraint (parent_pop, popsize);
    ...
    while (evaluation_count < max_evaluation)
    {
        ...
        assign_rank_constraint (mixed_pop, 2 * popsize);
        ...
        if(cv_size<popsize)
        {
            ...
            for (i = 0; i < 2 * popsize; i++)
            {
                ...
                {
                    temp[temp_count].idx = i;
                    temp[temp_count].x = mixed_pop->ind[i].cv;
                    temp_count ++;
                }
            }
        ...
        {
            fill_nd_pop_constraint (mixed_pop, parent_pop);

            getExtremePoints_constraint (candidate_pop, num_candidates);
            getIntercepts_constraint (candidate_pop, num_candidates);

            association_constraint (candidate_pop, num_candidates);
            niching_constraint (candidate_pop, parent_pop);
